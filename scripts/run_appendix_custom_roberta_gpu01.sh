...
for rk in 16 32; do
  out="${SUITE_ROOT}/appendix/rank/multi_glue3_${BACKBONE}_r${rk}_lora_e4_k2"
  label="appendix/rank/multi_glue3/${BACKBONE}/r${rk}/e4/k2"
  run_pipeline "$label" "$out" \
    ...
    --set "train.eval_train_fraction=0.10" \
    --set "train.eval_train_max_batches=-1" \
    --set "train.eval_max_batches=-1"
done

echo "[appendix/more-expert] start"
for mix in glue3 glue4; do
  case "$mix" in
    glue3) cfg="configs/multitask_glue3_rte_mrpc_cola_real.yaml" ;;
    glue4) cfg="configs/multitask_glue4_real.yaml" ;;
    *)
      echo "unsupported mix: $mix" >&2
      exit 1
      ;;
  esac
  out="${SUITE_ROOT}/appendix/more_expert/${mix}_${BACKBONE}_lora_e6_k3"
  label="appendix/more_expert/${mix}/${BACKBONE}/lora/e6/k3"
  run_pipeline "$label" "$out" \
    --config "$cfg" \
    --out_dir "$out" \
    --methods "baseline,ours" \
    --set "model.expert_type=lora" \
    --set "model.num_experts=6" \
    --set "model.top_k=3" \
    --set "train.eval_train_fraction=0.10" \
    --set "train.eval_train_max_batches=-1" \
    --set "train.eval_max_batches=-1"
...
for mix in glue3 glue4; do
  ...
  out="${SUITE_ROOT}/appendix/ffn_generalization/${mix}_${BACKBONE}_ffn_e4_k2"
  label="appendix/ffn_generalization/${mix}/${BACKBONE}/ffn/e4/k2"
  run_pipeline "$label" "$out" \
    --config "$cfg" \
    --out_dir "$out" \
    --methods "baseline,ours" \

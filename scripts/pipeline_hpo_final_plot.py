...
def _terminate_active_processes(phase_name: str, grace_seconds: float = 2.0) -> None:
    ...
    alive = [p for p in procs if p.poll() is None]
    if alive:
        print(f"[{phase_name}] warning: {len(alive)} process(es) still alive after SIGINT/SIGTERM; leaving them for manual inspection")
...
def _knob_env_token(key: str) -> str:
    out: List[str] = []
    for ch in str(key).upper():
        if ch.isalnum():
            out.append(ch)
        else:
            out.append("_")
...
def _centered_coordinate_values(
    *,
    kb: KnobSpec,
    trials_this_round: int,
) -> List[float] | None:
    if kb.scale != "log" or trials_this_round <= 1:
        return None

    tok = _knob_env_token(kb.key)
    center = _env_float(f"MOE_HPO_CENTER_{tok}")
    if center is None:
        return None

    lo = _env_float(f"MOE_HPO_LO_{tok}")
    hi = _env_float(f"MOE_HPO_HI_{tok}")
    lo_v = float(kb.lo if lo is None else lo)
    hi_v = float(kb.hi if hi is None else hi)
    if lo_v <= 0.0 or hi_v <= 0.0 or lo_v >= hi_v:
        return None

    center_v = float(min(max(center, lo_v), hi_v))
    if center_v <= 0.0:
        return None

    n = int(trials_this_round) - 1
    left_n = n // 2
    right_n = n - left_n
    log_lo = math.log(lo_v)
    log_hi = math.log(hi_v)
    log_center = math.log(center_v)

    left: List[float] = []
    if left_n > 0 and center_v > lo_v:
        for i in range(left_n):
            frac = float(i + 1) / float(left_n + 1)
            left.append(float(math.exp(log_center + frac * (log_lo - log_center))))

    right: List[float] = []
    if right_n > 0 and center_v < hi_v:
        for i in range(right_n):
            frac = float(i + 1) / float(right_n + 1)
            right.append(float(math.exp(log_center + frac * (log_hi - log_center))))

    vals: List[float] = []
    for i in range(max(len(left), len(right))):
        if i < len(left):
            vals.append(float(kb.clip(left[i])))
        if i < len(right):
            vals.append(float(kb.clip(right[i])))
    return vals[:n]
...
def _build_coordinate_candidates(
    *,
    ...
    centered_vals = _centered_coordinate_values(kb=kb, trials_this_round=trials_this_round)
    idx = 0
    if centered_vals is not None:
        for val in centered_vals:
            if len(out) >= trials_this_round:
                break
            cand = {k: float(v) for k, v in base_params.items()}
            cand[kb.key] = float(val)
...
def _to_set_args(
    *,
    ...
    steps: int,
    eval_every: int,
    extra_set: Sequence[str] = (),
) -> List[str]:
    kvs: List[str] = []
    kvs.extend(list(cli_set))
    kvs.extend(list(extra_set))
...
def _worker_signal_handler(signum: int, frame: Any, *, phase_name: str) -> None:  # noqa: ARG001
    _STOP_EVENT.set()
    _terminate_active_processes(phase_name=phase_name, grace_seconds=1.0)
    raise KeyboardInterrupt(f"worker interrupted by signal {signum}")


def _run_job_with_retry_process_entry(
    job: RunJob,
    config: str,
    gpu: int,
    retries: int,
    *,
    ...
    signal.signal(signal.SIGINT, lambda signum, frame: _worker_signal_handler(signum, frame, phase_name=phase_name))
    signal.signal(signal.SIGTERM, lambda signum, frame: _worker_signal_handler(signum, frame, phase_name=phase_name))
    try:
...
def _terminate_process_pool(ex: ProcessPoolExecutor, phase_name: str) -> None:
    processes = getattr(ex, "_processes", {}) or {}
    if not processes:
        return
    print(f"[{phase_name}] interrupt: stopping {len(processes)} worker process(es)")
    ...
    try:
        while pending or fut_meta:
            while pending:
                ...
                fut = ex.submit(
                    _run_job_with_retry_process_entry,
                    job,
                    config,
                    chosen_gpu,
                    retries,
            ...
            for fut in done_set:
                ...
                try:
                    ...
                    if failed_jobs >= max(1, int(max_failed_jobs)):
                        ...
                        for ff in fut_meta:
                            ff.cancel()
                        _terminate_process_pool(ex, phase_name=phase_name)
                        raise RuntimeError(
...
def main() -> None:
    ...
    p.add_argument("--hpo_trials", type=int, default=96, help="legacy: total coordinate trials per method (split across knobs)")
    p.add_argument("--coord_trials_per_knob", type=int, default=0, help="if >0, use fixed coordinate trials per knob (overrides --hpo_trials)")
    ...
    p.add_argument("--hpo_eval_val_fraction", type=float, default=1.0)
    ...
    p.add_argument("--skip_hpo_exports", action="store_true")
    p.add_argument("--skip_final_plotters", action="store_true")
    ...
    hpo_extra_set: List[str] = [f"train.eval_val_fraction={float(args.hpo_eval_val_fraction):.6g}"]
    if bool(args.hpo_skip_train_eval):
        hpo_extra_set.append("train.skip_train_eval=true")
    ...
    manifest = {
        ...
        "coord_trials_per_knob": int(args.coord_trials_per_knob),
    ...
    try:
        for mi, spec in enumerate(specs):
            ...
            with _file_lock(
                ...
                if int(args.coord_trials_per_knob) > 0:
                    trial_alloc = [int(args.coord_trials_per_knob)] * int(len(spec.knobs))
                else:
                        ...
                        jobs_round.append(
                            RunJob(
                                ...
                                set_kv=_to_set_args(
                                    ...
                                    seed=int(sd),
                                    steps=int(args.hpo_steps),
                                    eval_every=int(args.hpo_eval_every),
                                    extra_set=hpo_extra_set,
                                ),
                            )
                        )
                        order += 1
        ...
        if not bool(args.skip_hpo_exports):
        ...
        plotter_error = None
        if not bool(args.skip_final_plotters):
            try:
                _run_plotters(

...
for expert in lora ffn; do
  out="${SUITE_ROOT}/appendix/ffn_generalization/multi_glue3_${BACKBONE}_${expert}"
  python scripts/pipeline_hpo_final_plot.py \
...
python scripts/pipeline_hpo_final_plot.py \
  ...
  --out_dir "${SUITE_ROOT}/appendix/multi_dataset_real/${BACKBONE}" \
  --methods "baseline,ours" \
  --gpus "$GPUS" \
  --hpo_seeds "$HPO_SEEDS" \
  --final_seeds "$FINAL_SEEDS" \

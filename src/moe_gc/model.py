...
class MoEClassifier(nn.Module):
    def __init__(self, model_cfg: Dict[str, object]):
        ...
        if backend == "tiny":
            ...
            hf_local_files_only = bool(model_cfg.get("hf_local_files_only", False)) or _offline_env_enabled()
    ...
    def _head_input_dtype(self) -> torch.dtype:
        return self.router.weight.dtype

    def _cast_head_input(self, h: torch.Tensor) -> torch.Tensor:
        target_dtype = self._head_input_dtype()
        if h.dtype != target_dtype:
            h = h.to(dtype=target_dtype)
        return h

    def route_probs_from_hidden(self, h: torch.Tensor) -> torch.Tensor:
        h = self._cast_head_input(h)
    ...
    def _combine_logits(self, h: torch.Tensor, probs: torch.Tensor) -> torch.Tensor:
        h = self._cast_head_input(h)
        if probs.dtype != h.dtype:
            probs = probs.to(dtype=h.dtype)
        if self.expert_type == "ffn":
    ...
    def forward(
        ...
        head_h = self._cast_head_input(h)
        probs = self.route_probs_from_hidden(head_h)
        logits = self._combine_logits(head_h, probs)

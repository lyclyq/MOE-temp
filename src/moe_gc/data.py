    ...
    *,
    ...
    for candidate in (
        ...
        if candidate and Path(candidate).exists():
            datasets_cache_dir = candidate
            break

    load_kwargs: Dict[str, Any] = {}
    if datasets_cache_dir:
        load_kwargs["cache_dir"] = datasets_cache_dir
    ds = load_dataset("glue", task, **load_kwargs)
...
def _normalize_textcls_task(name: str) -> str:
    ...
    if s.startswith("hf_textcls/"):
        s = s.split("/", 1)[1]
    if s.startswith("local_textcls/"):
        suffix = s.split("/", 1)[1]
        local_aliases = {
            "sst2": "local_textcls/glue_sst2",
            "glue/sst2": "local_textcls/glue_sst2",
            "glue_sst2": "local_textcls/glue_sst2",
            "imdb": "local_textcls/imdb",
            "yelp": "local_textcls/yelp_polarity",
            "yelp_polarity": "local_textcls/yelp_polarity",
            "amazon": "local_textcls/amazon_polarity",
            "amazon_polarity": "local_textcls/amazon_polarity",
        }
        if suffix not in local_aliases:
            raise RuntimeError(f"unsupported local text classification dataset: {name!r}")
        return local_aliases[suffix]
    aliases = {
        "sst2": "glue/sst2",
        "glue/sst2": "glue/sst2",
        "glue_sst2": "glue/sst2",
...
def _textcls_spec(name: str) -> Dict[str, Any]:
    ...
    if task == "local_textcls/glue_sst2":
        return {
            "task": task,
            "dataset": "local_textcls",
            "subset": "glue_sst2",
            "train_split": "train",
    ...
    if task == "local_textcls/yelp_polarity":
        return {
            "task": task,
            "dataset": "local_textcls",
            "subset": "yelp_polarity",
            "train_split": "train",
            "val_split": "test",
            "text_a": "text",
            "text_b": None,
    ...
    if task == "local_textcls/amazon_polarity":
        return {
            "task": task,
            "dataset": "local_textcls",
...
def _build_textcls_task_loader(
    *,
    ...
    subset = spec["subset"]
    dataset_format = str(spec.get("dataset_format", "hf"))
    if dataset_format == "disk":
        local_root = os.path.expanduser(str(dcfg.get("local_dataset_root", "local_datasets/textcls")))
        ds = load_from_disk(os.path.join(local_root, str(subset)))
    else:

...
def _evaluate_metrics(
    ...
    with torch.no_grad():
        for task_name, loader in loaders.items():
            ...
            n_batches = 0
            loss_sum = 0.0
            task_batch_limit = int(max_batches_per_task)
            if task_batch_limit <= 0 and 0.0 < float(sample_fraction) < 1.0:
                try:
                    est_batches = int(len(loader))
                except Exception:
                    est_batches = 0
                if est_batches > 0:
                    task_batch_limit = max(1, int(math.ceil(float(est_batches) * float(sample_fraction))))
            for input_ids, attention_mask, labels in loader:
                if task_batch_limit > 0 and n_batches >= task_batch_limit:
                    break
                batch_cpu = {
                    "input_ids": input_ids,
                    "attention_mask": attention_mask,
...
def train(cfg: dict, data: MultiTaskData, model: MoEClassifier) -> TrainSummary:
    ...
    eval_train_max_batches = int(cfg["train"].get("eval_train_max_batches", -1))
    eval_train_fraction = float(cfg["train"].get("eval_train_fraction", 0.10))
    eval_train_fraction = min(1.0, max(0.0, eval_train_fraction))
    eval_val_fraction = float(cfg["train"].get("eval_val_fraction", 1.0))
    eval_val_fraction = min(1.0, max(0.0, eval_val_fraction))
    skip_train_eval = bool(cfg["train"].get("skip_train_eval", False))
    ...
    for step in range(1, steps + 1):
        ...
        if should_eval:
            val_m = _evaluate_metrics(
                ...
                sample_fraction=eval_val_fraction,
            )
            if skip_train_eval:
                tr_m = {"acc": float("nan"), "loss": float("nan")}
            else:
                train_eval_batches = eval_train_max_batches
                train_eval_fraction = eval_train_fraction
                if train_eval_batches < 0:
                    train_eval_batches = eval_max_batches
                    # If eval_max_batches is unset (full), train metrics default to 10% sampling.
                    # When eval_max_batches is set explicitly, follow that cap and disable fraction fallback.
                    if train_eval_batches > 0:
                        train_eval_fraction = 1.0
                tr_m = _evaluate_metrics(
                    ...
                    logical_micro_batch_size=logical_micro_batch_size,
                    device_batch_size=device_streaming_chunk_size,
                    max_batches_per_task=train_eval_batches,
                    sample_fraction=train_eval_fraction,
                )
